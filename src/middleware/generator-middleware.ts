 ...
 * sequences.
 */
const reduxMiddleware: Middleware =
  ({ dispatch }) =>
  (next) =>
  <
...
type DispatchCompatible<Dispatch extends (...args: any) => any> =
...
>(
  ...
  while (true) {
    const result: IteratorResult<Action, ReturnType> =
      iterator.next(prevDispatchResult);
